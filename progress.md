...
| 6. Protocol follow-up                      | in progress | The engine now emits exact session/model status metadata so the TUI can stop guessing session titles and context limits, session metadata saves no longer drop generated titles, and permission prompts carry structured target/access metadata. Rate-limit data and any explicit cost-threshold state are still pending.                                    |
...
- Remaining in Phase 6: rate-limit metadata and any explicit cost-threshold state still need dedicated engine/UI support.
